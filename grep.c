...
int main(int argc, char* argv[]) {
    char searchforthis[MAXCHAR];        //String to look for
    char inthisfile[MAXCHAR];       //File to check
    memset(searchforthis, '\0', sizeof(searchforthis));     //Initialize buffer
    memset(inthisfile, '\0', sizeof(inthisfile));   //Initialize buffer
    if(argc > 0) {
        if (strcmp(argv[1], "grep") == 0) {
            char* ptr = argv[2];
            if (*ptr == '\'') {     //To get what is within the ' -- SEARCH FOR THIS
                for(int i = 0; *ptr != '\''; ++i) {
                    searchforthis[i] = *ptr;
                    ++ptr;
                }
            }
            else { strcpy(searchforthis, argv[2]); }        //If no ' used
            strcpy(inthisfile, argv[3]);        //FILE NAME
        }
    }
    return 0;
}

//DO: CHANGE MAIN SO THE UI IS LIKE GREP, NOT ED
//int main(int argc, char *argv[]) {
//    char *p1, *p2;
//    SIG_TYP oldintr;
//
